...
pfasCorrrectedMOE <- function() {
  ...
  print(file)
  part = read.xlsx(file)
  cat("chemicals with partition data",length(unique(part$dtxsid)),"\n")
  ...
  print(file)
  qc = read.xlsx(file)
  cat("chemicals with in qc",length(unique(qc$dtxsid)),"\n")
  ...
  ivpod$qc = NA
  for(i in 1:nrow(ivpod)) {
    dtxsid = ivpod[i,"dtxsid"]
    if(is.element(dtxsid,qc$dtxsid)) {
      dqc = unique(qc[is.element(qc$dtxsid,dtxsid),"grade"])
      if(length(dqc)>1) ivpod[i,"qc"] = "P/F"
      else ivpod[i,"qc"] = dqc
    }
  }
  ...
  chems2 = unique(ivpod2[,c("dtxsid","casrn","name","nickname","qc")])
  ...
  for(i in 1:nrow(res)) {
    ...
    res[i,"moe"] = res[i,"pod_min_ngmL"] / res[i,"plasma_conc_ngmL"]
