...
#-------------------------------------------------------------------------------
ACToR.parse.fillin <- function(datasets=c("Denmark","Germany","HealthCanada","Japan","Michigan","Norway_Nordic","Ohio","Sweden")) {
#ACToR.parse.fillin <- function(datasets=c("Sweden")) {
  ...
  file = "data/PFAS Blood assay master list 2023-01-11 FINAL.xlsx"
  amaster = read.xlsx(file)
  amaster = amaster[amaster$useme=="red",]
  alldata = NULL
  for(dataset in datasets) {
    ...
    assays = assays[is.element(assays$snaid,amaster$source_name_aid),]
      ...
      for(j in 1:nrow(ba)) {
        ...
        if(is.element(snsid,sa$SOURCE_NAME_SID)) {
          ...
          for(k in 2:ncol(ba)) {
            acid = names(ba)[k]
            #browser()
            acname = atemp[is.element(atemp$acid,acid),"acname"][1]
            acdesc = atemp[is.element(atemp$acid,acid),"acdesc"][1]
            acunit = atemp[is.element(atemp$acid,acid),"acunit"][1]
            acvtype = atemp[is.element(atemp$acid,acid),"acvtype"][1]
  ...
  file = paste0("data/","all dataset data.xlsx")
